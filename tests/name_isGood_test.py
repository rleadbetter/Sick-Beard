...
def test_generator(toTest, tvdb_id, parseResult):
    def test(self):
...
for t in simple_test_cases:
    for cur_test_base in simple_test_cases[t]:
        if cur_test_base in dontTest:
            continue
        saneBase = sickbeard.helpers.sanitizeSceneName(cur_test_base)

        test_name = 'test_%s_%s' % (t, cur_test_base)

        if not simple_test_cases[t][cur_test_base]:
            continue

        testFunc = test_generator(cur_test_base, tvdb_id, simple_test_cases[t][cur_test_base])
